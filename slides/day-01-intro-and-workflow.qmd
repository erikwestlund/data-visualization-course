...
subtitle: "Introduction to Data Visualization & Workflow"
...
2. Understand course workflow with GitHub, RStudio/Positron
3. Use grammar-of-graphics vocabulary to build and read a first `ggplot2` plot.
...
ggplot(count_example, aes(x = category, y = n)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  coord_flip() +
  labs(
    title = "How Many Are In Each Category?",
    x = NULL,
    y = "Count"
  ) +
...
#| warning: false
set.seed(101)
distribution_example <- data.frame(value = rlnorm(500, meanlog = 3, sdlog = 0.45))

ggplot(distribution_example, aes(x = value)) +
  geom_histogram(bins = 30, fill = "gray75", color = "white") +
  labs(
    title = "What Does This Variable Look Like?",
    x = "Value",
    y = "Count"
  ) +
  theme_minimal(base_size = 18)
...
#| warning: false
set.seed(102)
association_example <- data.frame(
  x = runif(250, 0, 10)
) |>
  mutate(y = 2 + 0.7 * x + rnorm(n(), sd = 1.2))

ggplot(association_example, aes(x = x, y = y)) +
  geom_point(alpha = 0.65) +
  geom_smooth(se = FALSE, color = "steelblue") +
  labs(
    title = "How Do Two Variables Move Together?",
    x = "Variable X",
    y = "Variable Y"
  ) +
  theme_minimal(base_size = 18)
...
- interpretation and communication grab bag
:::
...
## AI/LLMs in this Course

- In this course, we will use code to learn "good visualization judgment,"" not just to make software produce an image.
- You can use LLMs to help you make visualizations, but do not use them to "do everything for you."
- The more you know and can do on your own, the more useful LLMs are to help you work efficiently and focus on substance over technical matters.
...
- The course site has key documents, links, and announcements
...
- Windows users may need to install Git separately: <https://git-scm.com/install/windows>
...
- `modules/01-workflow-and-basics/03_ggplot2-dictionary-and-cheatsheet.qmd`
- `modules/01-workflow-and-basics/04_dplyr-tidyr-dictionary-and-cheatsheet.qmd`
